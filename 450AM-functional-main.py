# main.py - v3.1 with Upgraded Agent Directives
import os
import json
import requests
import asyncio
...
2. Preserve Creative Intent: When delegating a task, you MUST include the user's full, original creative request. Do not abridge or over-simplify the core subject. Your role is to translate, not to edit.
3. OUTPUT FORMAT (STRICT): Your entire response must be ONLY the following JSON object and nothing else.
EXAMPLE OUTPUT:
{
...
4.  **Emphasize Key Subjects:** For prompts with multiple subjects (like a character AND a vehicle), use prompt weighting to emphasize the most important elements, like `(Phillie Phanatic:1.3)`. Describe the composition, for example: "The character is in the foreground, and the vehicle is behind them."
5.  **BE CONCISE:** The final positive prompt must be under 75 tokens.
6.  **OUTPUT FORMAT (STRICT):** Your entire response must be ONLY the following JSON object containing "final_prompt" and "negative_prompt".
"""
}
